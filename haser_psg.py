...
# Read base PSG config
server = 'https://psg.gsfc.nasa.gov'
f = open("config_base.txt", "r"); cfg = f.readlines(); f.close()
cfg[2] = '<GEOMETRY-OBS-ALTITUDE>%e\n' % distance
cfg[3] = '<OBJECT-DIAMETER>%.1f\n' % diameter
cfg[4] = '<OBJECT-STAR-DISTANCE>%.1f\n' % rh
cfg[5] = '<ATMOSPHERE-PRESSURE>%e\n' % Qgas
cfg[6] = '<ATMOSPHERE-ABUN>%e,%e\n' % (tgas1[0],tgas2[0])
cfg[7] = '<ATMOSPHERE-TAU>%e %e, %e %e\n' % (tgas1[1],tgas1[2],tgas2[1],tgas2[2])
updatePSG = True
...
while b<bmax:
    ...
    file = 'offset/rad-%.1f.txt' % b
    if not os.path.exists('offset'): os.system('mkdir offset')
    if not os.path.exists(file) or updatePSG:
        cfg[0] = '<GENERATOR-BEAM>%.1f\n' % fov
        cfg[1] = '<GEOMETRY-OFFSET-NS>%.1f\n' % b
        f = open("config.txt", "w")
        for line in cfg: f.write(line)
        f.close()
...
while b<bmax:
    ...
    file = 'width/rad-%.1f.txt' % b
    if not os.path.exists('width'): os.system('mkdir width')
    if not os.path.exists(file) or updatePSG:
        cfg[0] = '<GENERATOR-BEAM>%.1f\n' % b
        cfg[1] = '<GEOMETRY-OFFSET-NS>0.0\n'
        f = open("config.txt", "w")
        for line in cfg: f.write(line)
        f.close()
        os.system('curl -s --data-urlencode file@config.txt %s/api.php > %s' % (server,file))
...
ax[0,0].plot(fovs,np.asarray(p1)/np.asarray(areas),label=r'PSG - Water $H_{2}O$',linestyle='--')
ax[0,0].plot(fovs,np.asarray(p2)/np.asarray(areas),label=r'PSG - Hydroxyl $OH$',linestyle='--')
...
ax[0,1].plot(fovs,p1,label=r'PSG - Water $H_{2}O$',linestyle='--')
ax[0,1].plot(fovs,p2,label=r'PSG - Hydroxyl $OH$',linestyle='--')
...
ax[1,0].plot(offs,np.asarray(s1)/afov,label=r'PSG - Water $H_{2}O$',linestyle='--')
ax[1,0].plot(offs,np.asarray(s2)/afov,label=r'PSG - Hydroxyl $OH$',linestyle='--')
...
ax[1,1].plot(offs,s1,label=r'PSG - Water $H_{2}O$',linestyle='--')
ax[1,1].plot(offs,s2,label=r'PSG - Hydroxyl $OH$',linestyle='--')
